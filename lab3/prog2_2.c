  ...
  int AorB;
{
  ...
  printf("Pop pacote %d do buffer: ", packet.seqnum);
  for (int i = 0; i < 20; i++) {
    printf("%c", packet.payload[i]);
  }
  printf("\n");
  ...
  struct pkt packet;
{
  ...
  if (packet.checksum == checksum) { // veio nao corrompido
    ...
    if (packet.acknum == 0) { // normal message
      ...
      printf("Parei timer do A\n");
      stop_buffertimer(0);

      if (!isEmpty_window(0)) {
        printf("Comecei timer do A para pacote %d\n", A_expected_acknum);
  ...
  struct pkt packet;
{
  ...
  if (packet.checksum == checksum) { // veio nao corrompido
    ...
    if (packet.acknum == 0){ // normal message
      ...
      if (!isEmpty_window(1)) {
        printf("Comecei timer do B para pacote %d\n", B_expected_acknum);
