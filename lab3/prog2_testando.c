  ...
  struct pkt packet;
{
  ...
  if (packet.checksum == checksum) { // veio nao corrompido
    ...
    if (packet.acknum == 0){ // normal message
      ...
      if (A_last_seqnum_received != packet.seqnum) { // nao recebeu msg duplicada
        printf("Pacote nao duplicado -> envia para layer5\n");
        A_last_seqnum_received = packet.seqnum;
        tolayer5(0, packet.payload);
      } else {
        printf("Pacote duplicado -> ignora\n");
      }
    ...
    if (A_sent_first == 0) { // se recebi dados corrompidos
      struct pkt nack;
  ...
  struct pkt packet;
{
  ...
  if (packet.checksum == checksum) { // veio nao corrompido
    ...
    if (packet.acknum == 0){ // normal message
      ...
      if (B_last_seqnum_received != packet.seqnum) { // se nao veio msg duplicada
        printf("Pacote nao duplicado -> envia para layer5\n");
    ...
    // veio corrompido
    printf("Pacote %d corrompido\n", packet.seqnum);
    if (B_sent_first == 0) { // se veio dado corrompido
      struct pkt nack;
      nack.seqnum = packet.seqnum;
      nack.acknum = -packet.seqnum;
      for (int i = 0; i < 20; i++) {
  ...
  struct msg message;
{
  ...
  if (A_sending_message == 0) { // se nao tem msg sendo enviada
    A_sending_message = 1;
    A_sent_first = 1;
  ...
  struct msg message;
{
  ...
  if (B_sending_message == 0) { // se nao tem msg sendo enviada
    B_sending_message = 1;
    B_sent_first = 1;
